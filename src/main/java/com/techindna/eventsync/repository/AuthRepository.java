...
public class AuthRepository {
  ...
  public Optional<Administrator> findAdminDataByEmail(String email) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(sql)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        return rs.next()
            ? Optional.of(UserMapper.mapResultSetToAdministrator(rs))
            : Optional.empty();
      }
  ...
  public Participant saveParticipant(String firstName, String lastName, String email) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(insertQuery)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        return rs.next() ? UserMapper.mapResultSetToParticipant(rs) : new Participant();
      }
    } catch (SQLException e) {
      throw new InternalServerErrorException("Database error: " + e.getMessage());
    } finally {
  ...
  public Optional<Participant> findParticipant(String email, String firstName, String lastName) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(selectQuery)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        return rs.next() ? Optional.of(UserMapper.mapResultSetToParticipant(rs)) : Optional.empty();
      }
    } catch (SQLException e) {
      throw new InternalServerErrorException("Database error: " + e.getMessage());
    } finally {
  ...
  public Optional<Integer> findBlacklistedIp(String ipAddress) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(query)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        return rs.next() ? Optional.of(rs.getInt("failed_attempt")) : Optional.empty();
      }
    } catch (SQLException e) {
      throw new InternalServerErrorException("Database error: " + e.getMessage());
    } finally {
  ...
  public int incrementFailedAttempt(String ipAddress) {
    ...
    try (PreparedStatement ps = conn.prepareStatement(query)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        return rs.next() ? rs.getInt("failed_attempt") : MAX_ATTEMPT_LIMIT;
      }
    } catch (SQLException e) {
      throw new InternalServerErrorException("Database error: " + e.getMessage());
    } finally {
