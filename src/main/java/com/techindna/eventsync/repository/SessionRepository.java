...
public class SessionRepository {
    ...
    public Session saveSession(String title, String description, Instant startDate,
    ...
    public void addSpeakerToSession(UUID sessionId, UUID speakerId, LocalTime startTime, LocalTime endTime) {
        ...
        try (
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    throw new RuntimeException("Failed to add speaker to session");
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public boolean interveneExists(UUID sessionId, UUID speakerId) {
        final String query =
        """
        select 1 from eventsync_app.intervene
    ...
    public void updateSpeakerInSession(UUID sessionId, UUID speakerId, LocalTime startTime, LocalTime endTime) {
        ...
        try (
            ...
            ps.setObject(4, speakerId);

            int rows = ps.executeUpdate();
            if (rows == 0) {
                throw new NotFoundException(
                    String.format("Speaker %s not found in session %s.", speakerId, sessionId));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
