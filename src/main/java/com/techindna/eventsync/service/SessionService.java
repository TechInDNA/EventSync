...
public class SessionService {
  ...
  public SessionResponseDto createSession(SessionRequestDto sessionRequestDto) {
    ...
    Room room =
        roomRepository
            .findRoomByName(sessionRequestDto.getRoomName())
            .orElseThrow(() -> new NotFoundException("Room not found."));
    Event event =
        eventRepository
            .findEventByTitle(sessionRequestDto.getEventTitle())
            .orElseThrow(() -> new NotFoundException("Event not found."));

    return sessionRepository
        .createSession(sessionRequestDto, room, event)
        .orElseThrow(
            () ->
                new ConflictException(
                    String.format(
                        "Session with title '%s' already exists", sessionRequestDto.getTitle())));
  }

  @Transactional(readOnly = true)
  public GetSessionListResponseDto getAllSessions(
      GetSessionRequestDto request, String page, String size, String ipAddress) {
    ...
    PaginationRequestDto pagination =
        new PaginationRequestDto(Integer.parseInt(page), Integer.parseInt(size));
  ...
  public SessionResponseDto updateSession(String id, SessionRequestDto session) {
    ...
    Room room =
        roomRepository
            .findRoomByName(session.getRoomName())
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format("Room %s not found.", session.getRoomName())));
    Event event =
        eventRepository
            .findEventByTitle(session.getEventTitle())
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format("Event %s not found.", session.getEventTitle())));

    return sessionRepository
        .updateSessionById(UUID.fromString(id), session, room, event)
  ...
  public SessionDetailResponseDto getSessionById(String id, String ipAddress) {
    ...
    List<QuestionResponseDto> questions =
        questionRepository.getQuestionsBySessionId(UUID.fromString(id));
    return sessionRepository
        .findSessionDetailById(UUID.fromString(id), questions)
        .orElseThrow(() -> new NotFoundException(String.format("Session %s not found.", id)));
  }
  ...
  public void deleteSessionById(String id) {
    dataValidator.validateUUID(id);
    sessionRepository
        .deleteSessionById(UUID.fromString(id))
        .orElseThrow(() -> new NotFoundException(String.format("Session %s not found.", id)));
  }

  @Transactional
  public String updateSpeakerLink(
      String sessionId, String speakerId, SessionSpeakerInputDto input) {
    ...
    sessionRepository
        .updateSpeakerLink(sessionUUID, speakerUUID, input.getStartTime(), input.getEndTime())
        .orElseThrow(
            () ->
                new NotFoundException(
  ...
  public String addSpeakerToSession(
    ...
    sessionRepository.addSpeakerToSession(
        sessionUUID, speakerUUID, input.getStartTime(), input.getEndTime());
  ...
  public void removeSpeakerFromSession(String sessionId, String speakerId) {
    ...
    sessionRepository
        .removeSpeakerFromSession(sessionUUID, speakerUUID)
        .orElseThrow(
            () ->
                new NotFoundException(
                    String.format("Session %s or Speaker %s not found.", sessionId, speakerId)));
