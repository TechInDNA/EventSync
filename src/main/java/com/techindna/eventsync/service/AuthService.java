...
public class AuthService {
  ...
  @Transactional
  public Participant identifyOrRegisterParticipant(
      AuthParticipantRequestDto request, String ipAddress) {
    checkClient(ipAddress);
    dataValidator.validateParticipantData(request);

    Optional<Participant> participant =
        authRepository.findParticipant(
            request.getEmail(), request.getFirstName(), request.getLastName());
    return participant.orElseGet(
        () ->
            authRepository.saveParticipant(
