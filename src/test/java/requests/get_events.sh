...
curl -s http://localhost:8080/events | grep -o '"data":\[' | head -1
curl -s http://localhost:8080/events | grep -o '"meta":{' | head -1
...
curl -s http://localhost:8080/events | grep -o '"id":"[^"]*"' | head -1
curl -s http://localhost:8080/events | grep -o '"title":"[^"]*"' | head -1
curl -s http://localhost:8080/events | grep -o '"location":"[^"]*"' | head -1
...
curl -X POST -H "Content-Type: application/json" -d '{"title":"Test","description":"Test","startDate":"2026-06-01T10:00:00Z","endDate":"2026-06-01T18:00:00Z","location":"Test"}' -i http://localhost:8080/events
...
curl -H "Content-Type: application/json" "http://localhost:8080/events?invalid=param"
