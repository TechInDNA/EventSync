...
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Updated Room Name"}' http://localhost:8080/rooms/$ROOM_ID

# Step 4: Duplicate name (another room already has this name, should return 409)
echo ""
echo "--- Test 3: Duplicate name (should return 409) ---"
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Grand Ballroom"}' http://localhost:8080/rooms/$ROOM_ID

# Step 5: Room not found (should return 404)
echo ""
echo "--- Test 4: Room not found with non-existent UUID (should return 404) ---"
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"NonExistent Room"}' http://localhost:8080/rooms/$INVALID_UUID

# Step 6: Malformed UUID (should return 400)
echo ""
echo "--- Test 5: Malformed UUID (should return 400) ---"
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Malformed Uuid Room"}' http://localhost:8080/rooms/$MALFORMED_UUID

# Step 7: Missing name (should return 400)
echo ""
echo "--- Test 6: Missing name (should return 400) ---"
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{}' http://localhost:8080/rooms/$ROOM_ID

# Step 8: Blank name (spaces only, should return 400)
echo ""
echo "--- Test 7: Blank name (spaces only, should return 400) ---"
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"   "}' http://localhost:8080/rooms/$ROOM_ID
...
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Invalid!Room"}' http://localhost:8080/rooms/$ROOM_ID
...
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Valid Updated Room Name"}' http://localhost:8080/rooms/$ROOM_ID
...
curlie -X PUT -b cookies.txt -H "Content-Type: application/json" -d '{"name":"Valid.Room,Test:Name-Updated"}' http://localhost:8080/rooms/$ROOM_ID
